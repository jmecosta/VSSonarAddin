...
public class TextDiff {
    ...
    private void stretchOneMatch(int blockNum, int oldLineNum, int newLineNum, int whichWay) {
        ...
        while (true) {
            ...
            if (lOldLineNum > 0 && lOldLineNum < oldFileInfo.length && oldFileInfo.lines[lOldLineNum] != null) {
                ...
                if (lOldLineNum < oldFileInfo.length && lOldLineNum > 0 && oldFileInfo.lines[lOldLineNum] != null) {
                    if (!(oldFileInfo.isValidLineNum(lOldLineNum) && newFileInfo.isValidLineNum(lNewLineNum) && oldFileInfo.lines[lOldLineNum].equals(newFileInfo.lines[lNewLineNum]))) {
                        break;
                    }
                }
            } else {
